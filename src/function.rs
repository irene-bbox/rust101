...
fn printer(num:i32) { println!("{}", num); }

// A simple doubling function
fn double(int: &mut i32) { *int *= 2; println!("{}", int); }
...
fn yoda_greetings() { println!("Greetings from baby Yoda!")}
pub fn closures()
{
...
pub fn high_order_fn()
{
    ...
    {
        let sq = i * i;
        if sq >= limit { break; } else if is_even(sq) { sum += sq; }
    }
    ...
    fn greater_than(limit: i32) -> impl Fn(i32) -> bool { move |y| y > limit}
    ...
    {
        let sq = i * i;
        if above_limit(sq) { break; } else if is_even(sq) { sum2 += sq; }
    }
