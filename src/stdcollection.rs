...
pub fn hashset()
{
    ...
    if removed == true { println!("Removal succeeded") };
    print!("greek = {:?}", greek);
    ...
    if subset == true { println!("Yes, it's a subset") };
    println!("{:?} is a subset of {:?}? {}", OneToFive, OneToTen, subset);

    // Disjoint =  no common elements
    let disjoint = OneToFive.is_disjoint(&FourToNine);
    println!("{:?} is disjoint from {:?}? {}", OneToFive, FourToNine, disjoint); // always notice that the data output is entirely out of order

    // Union
    let union = OneToFive.union(&FourToNine);
    println!("The union of {:?} and {:?} is = {:?}", OneToFive, FourToNine, union);

    // Intersection
    let intersection = OneToFive.intersection(&FourToNine);
    println!("The intersection of {:?} and {:?} is = {:?}", OneToFive, FourToNine, intersection);
    ...
    println!("Set {:?} - {:?} = {:?}", OneToFive, FourToNine, symm_difference);
}
...
pub fn iterators()
{
    ...
    for i in &vec { println!("{}", i) }; // iterate through the vector using a for loop
    for i in &vec { println!("{}", *i) };
    ...
    println!("vec = {:?}", vec);
    for j in vec.iter().rev() { println!("reversed vec = {}", j) }; // rev() for reverse iteration
