...
pub fn if_statement() {
    ...
    println!(
        ...
        if temp > 20 {
            if temp <= 25 {
                "hot, bwt 20-25 C degrees"
            } else {
                "hot, above 25 C degrees"
            }
        } else if temp < 10 {
            "cold"
        } else {
    ...
    // If statements satisfying multiple conditions are declared using '&&'
    let mut a: u8 = 3;

    if a % 2 == 0 && a >= 0 {
        println!("a is a non-negative even number")
    } else {
        println!("a is either negative or odd, or both")
    }
}
...
pub fn while_loop() {
    ...
    while x < 100 {
        ...
        if x == 32 {
            continue;
        } //the 'continue' command instructs the program not to execute the current line of code, but to just straight to the next line instead
        println!("x = {}", x);
    }
...
pub fn for_loop() {
    ...
    //for x in [1,10] inclusive
    {
        if z == 3 {
            continue;
        } //if z is 3, skip and continue
        if z == 1 << 3 {
            break;
        } //if z is equal to 2^3=8 which is 1000 in binary, break
        println!("z = {}", z);
    }

    // Let's practise with another for loop printing both the index and the value of the element
    for (index, w) in (20..30).enumerate()
    // .enumerate() is an iterator that returns a tuple (the current count, and the element during iteration)
    {
        println!("{}:{}", index, w);
    }
}

// 4 distinct ways to iterate through a Rust vector using a for loop
...
enum State {
    ...
    failed,
    unlocked,
} //enumerate the states the locker can be in as you first interact with it
